...
export class LocalProvider extends LLMProvider {
  ...
  async *stream(
    ...
    try {
      ...
      // Ollama doesn't always report token counts - estimate from chars
      const estimatedUsage: TokenUsage = {
        inputTokens: 0,
        outputTokens: Math.ceil(totalOutput / 4),
        totalTokens: Math.ceil(totalOutput / 4),
      };

      this.emitUsage(model, estimatedUsage, Date.now() - startTime);
      yield { type: 'done', usage: estimatedUsage };
    } catch (error) {
      this.emitError(error as Error, model);
      throw error;
    }
